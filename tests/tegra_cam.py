...
#
# db: since we have a tx1 with custom opencv3 build:
#   - found that gstreamer for: --usb --vid 0  fails
#   - modified the usb path to use non-gstreamer
#
# --------------------------------------------------------
...
def open_cam_usb(dev, width, height):
    ...
    if False:
        ...
        cap.set(cv2.CAP_PROP_FPS, 60) # db: 10 is definitely slower than 60
        cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1)
        #ret_val, displayBuf = cap.read()  # do we need one read before setting certain properties?
        return cap
